          ...
          child: Column(
            ...
            children: [
              ...
              Builder(builder: (context) {
                ...
                late final Map<String?, List<TransactionModel>> temp;

                if (_chartController.isExpense.value) {
                  temp = _homeController.myTransactions.where((element) => !element.isIncome).groupBy((p0) => p0.category);
                } else {
                  temp = _homeController.myTransactions.where((element) => element.isIncome).groupBy((p0) => p0.category);
                }

                if (temp.isEmpty) {
                  return Text("No Data added yet");
                }

                temp.forEach((key, value) {
                  String label = key.toString();
                  int totalCategoryAmount = value.map((e) => int.parse(e.amount!)).reduce((a, b) => a + b);
                  dataMap["$label: (${value.length})"] = totalCategoryAmount.toDouble();
                });
